...
function Download-File {
    ...
    try {
        ...
        # ใช้ BITS สำหรับไฟล์ใหญ่, Invoke-WebRequest สำหรับไฟล์เล็ก
...
if (Test-Path "$processingInstallDir\processing.exe") {
    Write-Log "Processing ลงแล้ว — ข้าม" "SUCCESS"
}
else {
    # Processing 4.x สำหรับ Windows 64-bit
    ...
    if ($downloaded) {
        Write-Log "กำลัง extract Processing..."
        try {
            ...
            Write-Log "❌ Extract Processing ล้มเหลว: $($_.Exception.Message)" "ERROR"
        }
    }
}
...
if ($pulsarCheck) {
    Write-Log "Pulsar ลงแล้ว — ข้าม" "SUCCESS"
}
else {
...
if (Test-Path "$eclipseInstallDir\eclipse.exe") {
    Write-Log "Eclipse ลงแล้ว — ข้าม" "SUCCESS"
}
else {
    # Eclipse 2024-09 CDT (C/C++ Development Tooling)
    ...
    if ($downloaded) {
        Write-Log "กำลัง extract Eclipse..."
        try {
            Expand-Archive -Path $eclipseZip -DestinationPath "C:\" -Force
            # Eclipse extracts to C:\eclipse by default
            ...
            Write-Log "❌ Extract Eclipse ล้มเหลว: $($_.Exception.Message)" "ERROR"
        }
    }
}
...
Write-Log "⚠️  Quartus ต้อง download manual — เปิด browser..." "WARNING"
Write-Host ""
...
Write-Log "⚠️  Packet Tracer ต้อง download manual — เปิด browser..." "WARNING"
Write-Host ""
Write-Host "  ╔══════════════════════════════════════════════════════════╗" -ForegroundColor Yellow
...
Write-Host "⚠️  ยังต้องลง Quartus + Packet Tracer ด้วยตนเอง" -ForegroundColor Yellow
